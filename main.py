...
def main():
    ...
    erase_powergrid_csv_data(os.path.join(SCRIPT_DIR, 'power_grid_datafiles/power_grid_input.csv'))
    erase_powergrid_csv_data(os.path.join(SCRIPT_DIR, 'power_grid_datafiles/power_grid_output.csv'))

    q1 = queue.Queue()
    qkd_run = threading.Event()

    # Create and start a thread for the forever loop
    forever_loop_thread = threading.Thread(target=monitor_csv_file, args=(os.path.join(SCRIPT_DIR, 'power_grid_datafiles/power_grid_input.csv'), 0.1, q1))
    forever_loop_thread.daemon = True  # Daemon thread exits when the main program exits
    forever_loop_thread.start()

    # Start user input thread to handle commands
    ...
    while True:
        ...
        if new_csv_row > current_csv_row:
            ...
            for i in range(current_csv_row , new_csv_row ):
                row_data = read_csv_nth_row(os.path.join(SCRIPT_DIR, 'power_grid_datafiles/power_grid_input.csv'), i)
